...
def capsule_neural_network(feature_sizes: list, input_feature: int, capsule_tall: int, capsule_wide: int):
    input_features = feature_sizes[0]*capsule_tall + 1 + input_feature
    assert input_features % capsule_tall == 0, f'{input_features} should divisible by {capsule_tall}'
    ...
    def forward_pass(input_batch: torch.Tensor):
        ...
        for capsule_wide_idx in range(capsule_wide):
            for layer_idx, layer in enumerate(layers):
                capsule_wide_idx_as_tensor = torch.full([input_batch.shape[0], 1], capsule_wide_idx, device="cuda")
                if layer_idx == 0:
                    input_for_layer = torch.concat([input_batch, previous_layer_output, capsule_wide_idx_as_tensor], dim=1)
                else:
                    input_for_layer = torch.concat([previous_layer_output, capsule_wide_idx_as_tensor], dim=1)
